  ...
  return(
    <div className="signin-screen" style={{background:"var(--bg)"}}>
      ...
      <div className="signin-card">
        ...
        <div style={{fontSize:"0.82rem",color:"var(--text3)",marginBottom:8}}>Agendas · Projects · YouTube Study Plan</div>
...
function ProjectsSection({projects,setProjects,gamify}){
  ...
  const addTask=(projId)=>{
    ...
    if(editTaskRef){
      setProjects(p=>p.map(proj=>proj.id===projId?{...proj,tasks:proj.tasks.map(t=>t.id===editTaskRef?{...t,title:taskInput,deadline:taskDeadline}:t)}:proj));
      setEditTaskRef(null);
    }else{
      setProjects(p=>p.map(proj=>proj.id===projId?{...proj,tasks:[...(proj.tasks||[]),{id:Date.now(),title:taskInput,deadline:taskDeadline,done:false}]}:proj));
    }
...
// ══════════════════════════════════════════════════════════════════════════════
function TodayDashboard({agendas,setAgendas,projects,setProjects,study,setStudy,gam,gamify}){
  // Collect ALL tasks across agendas and projects
  // Bug fixes: #1 only show today's completions, #2 skip completed agendas/projects entirely,
  // #6 show empty projects with upcoming deadline, #7 no-deadline tasks excluded, #9 skip completed projects
  const pendingItems=[];const completedTodayItems=[];
  (agendas||[]).forEach(ag=>{
    const agDone=ag.tasks?.length>0&&ag.tasks.every(t=>t.done);
    if(agDone)return; // bug fix #2
    (ag.tasks||[]).forEach(task=>{
      if(!task.deadline)return; // #7: no-deadline tasks stay out of Today
      const days=getDaysLeft(task.deadline);
      if(task.done){
        if(isToday(task.completedAt))completedTodayItems.push({source:"agenda",sourceTitle:ag.title,sourceId:ag.id,task,days}); // #1
      }else if(days!==null&&days<=0){
        pendingItems.push({source:"agenda",sourceTitle:ag.title,sourceId:ag.id,task,days});
      }
    });
    // Agenda-level deadline (no tasks)
    if(ag.tasks?.length===0&&ag.deadline){
      const days=getDaysLeft(ag.deadline);
      if(days!==null&&days<=0)pendingItems.push({source:"agenda",sourceTitle:ag.title,sourceId:ag.id,task:{id:"ag-"+ag.id,title:"(Agenda deadline)",done:false,deadline:ag.deadline},days,isAgendaShell:true});
    }
  });
  (projects||[]).forEach(proj=>{
    ...
    (proj.tasks||[]).forEach(task=>{
      ...
      const days=getDaysLeft(task.deadline);
      if(task.done){
        if(isToday(task.completedAt))completedTodayItems.push({source:"project",sourceTitle:proj.title,sourceId:proj.id,projColor:proj.color,task,days}); // #1
      }else if(days!==null&&days<=0){
        pendingItems.push({source:"project",sourceTitle:proj.title,sourceId:proj.id,projColor:proj.color,task,days});
      }
    });
    // #6 Project-level deadline (empty project or future tasks but project deadline is today/overdue)
    if((proj.tasks||[]).length===0&&proj.deadline){
      const days=getDaysLeft(proj.deadline);
      if(days!==null&&days<=0)pendingItems.push({source:"project",sourceTitle:proj.title,sourceId:proj.id,projColor:proj.color,task:{id:"proj-"+proj.id,title:"(Project deadline)",done:false,deadline:proj.deadline},days,isProjectShell:true});
    }
  });
  pendingItems.sort((a,b)=>(a.days??0)-(b.days??0));
  ...
  return(
    <div>
      ...
      <div className="stat-grid" style={{marginBottom:12}}>
        {[
          ["Pending",pendingItems.length,"tasks due","var(--red)"],
      ...
      <div className="two-col" style={{marginBottom:20,marginTop:18}}>
        {/* Pending tasks */}
        <div>
          <div className="section-label">PENDING TODAY<span style={{color:pendingItems.length>0?"var(--red)":"var(--text3)"}}>{pendingItems.length} remaining</span></div>
          {pendingItems.length===0?(
            <div style={{background:"var(--glass)",border:"1px solid var(--glass-border)",borderRadius:"var(--radius-sm)",padding:"12px 14px",textAlign:"center",fontSize:"0.78rem",fontFamily:"var(--mono)",color:"var(--green)"}}>✓ All clear</div>
          ):pendingItems.map((p,i)=>(
            <div key={i} className={`today-task${p.days!==null&&p.days<0?" overdue":""}`} onClick={()=>toggleTask(p)} style={{cursor:p.isAgendaShell||p.isProjectShell?"default":"pointer"}}>
              ...
              {p.task.deadline&&<DeadlineBadge dateStr={p.task.deadline} done={false} sm/>}
            </div>
          ))}
        </div>
        ...
        {/* Completed today */}
        <div>
          <div className="section-label">COMPLETED TODAY<span>{completedTodayItems.length} done</span></div>
          {completedTodayItems.length===0?(
            <div style={{background:"var(--glass)",border:"1px solid var(--glass-border)",borderRadius:"var(--radius-sm)",padding:"12px 14px",textAlign:"center",fontSize:"0.78rem",fontFamily:"var(--mono)",color:"var(--text3)"}}>Nothing completed yet</div>
          ):completedTodayItems.map((p,i)=>(
            <div key={i} className="today-task done-task" onClick={()=>toggleTask(p)}>
  ...
  useEffect(()=>{
    const unsub=onAuthChange(async(u)=>{
      ...
      if(u){
        ...
        setLoaded(true);
      }else{setLoaded(false);setAgendas([]);setCategories([]);setProjects([]);setStudy({entries:[],completed:{},playlists:[],maxLen:65,startDate:today(),apiKey:""});setGam(defaultGam());}
    });
    return unsub;
  },[]);

  const persist=useCallback((ag,cats,pr,st,gm)=>{
    if(!user)return;clearTimeout(saveTimer.current);setSyncing(true);
    saveTimer.current=setTimeout(async()=>{
      try{await saveUserData(user.uid,{agendas:ag,categories:cats,projects:pr,study:st,gam:gm});setLastSaved(new Date());}catch(e){console.error(e);}
      setSyncing(false);
    },1500);
  },[user]);

  useEffect(()=>{if(loaded)persist(agendas,categories,projects,study,gam);},[agendas,categories,projects,study,gam,loaded]);

  const handleSignIn=async()=>{setAuthLoading(true);try{await signInWithGoogle();}catch(e){console.error(e);}setAuthLoading(false);};
  const handleSignOut=async()=>{await signOutUser();setAgendas([]);setCategories([]);setProjects([]);setStudy({entries:[],completed:{},playlists:[],maxLen:65,startDate:today(),apiKey:""});setGam(defaultGam());setLoaded(false);};
  ...
  return(
    <div className="app-shell">
      ...
      <div className="app-content">
        ...
        <div className="page">
          {tab==="today"&&<TodayDashboard agendas={agendas} setAgendas={setAgendas} projects={projects} setProjects={setProjects} study={study} setStudy={setStudy} gam={gam} gamify={gamify}/>}
